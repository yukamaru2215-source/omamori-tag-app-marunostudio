...
function Label({ qrUrl }: { qrUrl: string }) {
  return (
    <div
      className="border border-dashed border-gray-300 bg-white flex items-center gap-[1.5mm] overflow-hidden"
      style={{ width: `${LABEL_W_MM}mm`, height: `${LABEL_H_MM}mm`, padding: '1.5mm', breakInside: 'avoid' }}
    >
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={qrUrl} alt="QR" className="flex-shrink-0" style={{ width: '19mm', height: '19mm' }} />
      <div className="min-w-0 leading-tight">
        <div className="text-[5px] font-bold text-[#4A6E55]">🏷️ おまもりタグ</div>
        <div className="text-[7.5px] font-black text-[#1A6640] mt-[0.3mm]">使い方ガイド</div>
        <div className="text-[5px] text-gray-500 mt-[0.3mm]">QRを読み取ってね</div>
      </div>
    </div>
  )
}
  ...
  return (
    <>
      ...
      <div className="no-print max-w-[960px] mx-auto px-10 pt-24 pb-4 text-sm text-gray-500 leading-relaxed">
        ...
        読み取り先は<span className="font-mono text-[#1A6640]">{targetUrl}</span>（画面つき使い方ガイド）です。
