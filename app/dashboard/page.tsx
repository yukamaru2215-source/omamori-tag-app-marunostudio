...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase.from('children').select('*').eq('parent_id', session.user.id)
      setChildren(data ?? [])
      setLoading(false)
    }
    load()
    ...
    <main className="min-h-screen bg-[#F4F7F5]">
      <div className="max-w-md mx-auto p-4 pb-16">
        <div className="flex items-center justify-between py-4 mb-4">
          ...
          <button onClick={handleLogout} className="text-sm text-[#7A8E80] border border-[#E0EAE2] px-3 py-2 rounded-xl bg-white">ログアウト</button>
        </div>
          ...
          children.map(child => (
            <div key={child.id} className="bg-white rounded-2xl p-4 border border-[#E0EAE2] shadow-sm mb-3">
              ...
              <div className="flex gap-2">
                <Link href={`/kid/${child.slug}`} className="flex-1 text-center bg-[#E6F4EC] text-[#1A6640] py-2 rounded-xl text-sm font-bold">情報を見る</Link>
                <Link href={`/edit/${child.id}`} className="flex-1 text-center bg-[#F4F7F5] text-[#3A4A3E] py-2 rounded-xl text-sm font-bold border border-[#E0EAE2]">編集する</Link>
              </div>
            </div>
          ))
        )}
        <Link href="/register" className="block w-full bg-[#1A6640] text-white text-center py-4 rounded-2xl font-bold text-lg shadow-lg mt-2">＋ お子様を登録する</Link>
      </div>
    </main>
  )
}